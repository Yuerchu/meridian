...
- **The transcript is bubbles, and a tool is a key on the keyboard under its bubble.** Drawn the way a messenger draws a chat: the person's bubbles on the right, the model's on the left in runs with one avatar, and a row of keys under a bubble for the calls it made — an inline keyboard, with the key's panel opening below the row. There is no collapsed "worked for 3s" header any more; nothing in a turn is folded away, only the panels are shut. Five things about it are not the obvious reading.
  ...
  **A key's panel is a bubble continuation, and what it shows is parsed out of a string.** `ChatToolContent` in keyboard mode is a `rounded-xl` box with `bg-[var(--bubble-assistant)]` — the same fill as the prose bubble above it, so a key opening detail reads as one message expanding rather than a card inside a card. The status ring is on this element, wrapping everything inside it; no Widget, no second layer. The blocks compose `ChatToolPanelHeader` / `ChatToolPanelBody` / `ChatToolPanelFooter`, which are plain `div`s with padding. The decision row *portals* into the footer (`ChatToolFooterSlot`): `PendingApproval` renders it deep under the notice and the reason field it belongs with, so the footer is a node made before the first render, occupied by whoever mounts into it, and drawn only then.
