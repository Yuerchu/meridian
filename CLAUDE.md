...
- **Right-click menus are Pro's `ContextMenu`, and base-ui is gone.** Four things about it that the docs do not say. `Trigger` spreads its props *after* its own handlers, so an `onContextMenu` on it replaces the one that opens the menu — the sidebar records which row was hit on `onPointerDown` and `onContextMenuCapture` instead. Its `render` is a function of the DOM props, not an element, and the ref in them has to reach a real node; the message groups are the triggers that way. The trigger is `inline-block`, so one wrapping a full-width child needs `block` — `#playground/heroui` measures whether that still wins. And it closes on any `scroll` outside the popover, from a window capture listener nothing can intercept: accepted, because the only thing that scrolls under an open menu without a hand on it is the transcript following a stream, when there is nothing in the menu worth doing. A `Root` parked away from its `Trigger` does not throw; it renders and never opens. `useContextMenuGuard` cancels the WebView's own menu everywhere except a touch screen and an editable field with no menu of ours; devtools is Ctrl+Shift+I.
