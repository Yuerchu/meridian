...
- **The conventions above are gated, not reviewed — and a gate has to be
  ...
  with nothing failing), and `no-silent-prop-drop` (a base component may not
  accept a prop and drop it as `_x` — the shape behind every dead primitive of
  the first boardui pass). Two are recurrence gates, each for a class of bug
  ...
  given under `boardui.json` above.
