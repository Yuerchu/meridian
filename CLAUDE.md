...
- **Settings is pages on a stack, at every width.** `settings/settings-stack.tsx` is the one navigation model: a list is a page, the thing you opened is a page over it, its sub-thing is a page over that. There used to be two — a list beside its detail above 536px of *measured pane* and a drilldown below it — which is two layouts, two sets of state to keep honest and a threshold to argue about. The stack replaced `useMasterDetail` for providers; `MasterDetail` and `SettingsDrilldown` survive only until MCP, assistants and the tool marketplace move over.
  ...
  **Each page fetches what it needs by id.** That is what removes the coordination: a list refetches on every return, so a rename or a delete is picked up with no callback threaded down and nothing to invalidate. Nothing auto-selects — opening a page nobody asked for puts a back button in front of somebody who has not gone anywhere.
...
- **A question waiting on a person is not part of a session.** `sessions[id].pendingApprovals` says which id a *card* is waiting for and is only ever written for a conversation somebody has opened — `ensureSession` runs when `ChatView` mounts, and `handleToolApproval` returned early without one. That is right for a card and wrong for everything else: with several conversations working at once, the ones that stop for permission are mostly ones nobody is looking at, so the event announcing them was being dropped entirely. The sidebar dot never lit for them either.
  ...
  **Order is ours, and reconciling it to `ToastQueue` needs two facts that the API hides.** `add` *unshifts* (`react-stately/dist/private/toast/useToastState.mjs`), so `visibleToasts[0]` is the row added *last*, and the queue is rebuilt back-to-front on every change of the *sequence*. Diffing by set instead is the trap: deferring the front of three visible rows leaves the same three ids on screen, so a set diff finds nothing to do and the button silently does nothing. The region (`components/base/toast.tsx`, React Aria's `UNSTABLE_ToastRegion`) draws every visible toast and all of them are pressable; how many are visible is `maxVisibleToasts` on the *queue's constructor*, which is the only place react-stately reads it.
  ...
  **Deferring is the only way past a row**, which is why there is no close button: the question is still owed either way, and a dismissal that ended a turn's only visible sign would make a mis-click expensive. The conversation being read is excluded, because its card is already at the live edge. ("Card" throughout this entry: since the transcript became bubbles, the card is the key's panel under the bubble, with the decision row at its top — the queue's side of the wall is unchanged.)
...
Built on a vendored [boardui](https://www.boardui.com/) snapshot under `src/components/base/`, with React Aria Components underneath every interactive primitive. **Front-end conventions follow boardui** unless there is a specific reason that requires human sign-off: semantic tokens only (`text-text-*`, `bg-background-*`, `border-border-*`, `accent-*`; no raw palette, no HeroUI-era names), the composite type scale (`text-body-medium`, never `text-sm font-medium`), `cx()` from `@/utils/cx` for class merging, the boardui motion recipe (`components/base/overlay-motion.ts`), and boardui's radius ladder. The `boardui` skill and MCP server are the reference; `npx boardui list` is the registry. What boardui does not have — status colours, the two bubble fills, scrollbars, safe-area insets, the cursor token, sheet keyframes — is `src/styles/meridian.css`, defined against boardui's primitives so a rebrand reaches it. The boardui accent is blue and the bubbles are deliberately not: who is speaking has its own tokens.

**The base layer's interaction contract is React Aria's, not boardui's.** The registry's `Button` is a native `<button>`; here it renders RAC's `Button` with boardui's classes, and takes `onPress` / `isDisabled` / `isPending` / `slot`. That is not taste: every trigger primitive in this app — `TooltipTrigger`, `MenuTrigger`, `DialogTrigger`, a `Dialog`'s `slot="close"` — hands its handlers and its ref down *through context*, and only a `usePress`/`useFocusable` consumer receives them. A native button in that position looks right, type-checks, and does nothing: no tooltip, no menu, a close button that closes nothing. The same rule made `Input`/`TextArea`/`SearchField.Input` RAC inputs (a bare `<input>` inside a `TextField` is labelled by nothing), `Disclosure` a RAC disclosure, `Sidebar.Menu` a RAC `Tree`, and `Command.Dialog` a RAC `Autocomplete`. Install a registry item fresh (`npx boardui add -d src <name>`), then adapt it; **never `--overwrite`** an adapted file. `src/components/base/base.contract.test.tsx` asserts these behaviours against the DOM, and the `no-silent-prop-drop` lint rule reports a base prop destructured into `_x` — the two gates that would have caught the 2026-09 shells. Pro items (`composer`, `agent-progress`, `questionnaire`, `web-search`, `task-list`, `agent-limits-card`) need a licence (`npx boardui login`) and are the planned replacements for `PromptInput`, `ChainOfThought`, `ChatSource`, the `ask_user` form and the context gauge.
...
- **The transcript is bubbles, and a tool call is one of them.** Drawn the way a messenger draws a chat: the person's bubbles on the right, the model's on the left in runs with one avatar — and each call it made in the same shell as the prose it made them for, a block in the same bubble taking the same fill, the same radius and the same corner treatment. A tool is something the assistant *did* while it was talking, so it is drawn as another thing it said. There is no collapsed "worked for 3s" header any more; nothing in a turn is folded away, only the details are shut. Five things about it are not the obvious reading.
  ...
  *No* — the fill, the text colour, the edge and the hover lift depend only on whose bubble this is, so they travel as four inherited custom properties (`--bubble-fill`, `--bubble-ink`, `--bubble-edge`, `--bubble-lift`) that a variant sets and `BUBBLE_BLOCK` reads. They reach a block at any depth, and one drawn outside any bubble falls back to the assistant's. These were `[&>[data-bubble-block]]:` rules once, and a block that picked up a wrapper lost its fill without a word.
...
- **Radius: boardui's ladder, and a child never rounder than the container that clips it.** Frame panels (sidebar, main) `rounded-3xl`; cards, menus, popovers, modals, the composer shell `rounded-2xl`; buttons `rounded-2lg` (10px); inputs, menu rows and tool blocks `rounded-lg`…`rounded-xl`; pills `rounded-full`. Overriding `h-*`/`px-*` on a Button without also overriding `rounded-*` is the usual way a hover fill gets clipped at the corners.
- **Component style:** `data-slot` on every DOM node in app code (the vendored base layer is exempt from that one rule), `cx()` with `className` last, `tv` from `tailwind-variants` (re-exported by the barrel) for variant recipes, `dom.*` with a `render` prop instead of `asChild`.
...
- **Nothing on screen is drawn by the browser.** A native `title` is the browser's own tooltip — its font, its delay, its corner — and it is banned by the lint on every intrinsic element (`iframe` excepted, where it is the frame's accessible name and draws nothing), as are `<details>`, `<progress>`, `<select>` and friends, and `alert`/`prompt`/`window.confirm`. For inert text with more to say — a truncated path, a figure's qualifier — `ui/hint.tsx` composes a `Tooltip` around a `Tooltip.Trigger` rendered *as* the text's own element, so there is no wrapper and the span is the trigger. `focusable={false}` where the same hint repeats many times over (a bar chart's segments): a tab stop per segment is worse than hover-only, which is all `title` ever gave them. A focusable element that needs one — a link — renders itself through `Tooltip.Trigger`'s `render` and puts its `role` back, since the trigger defaults to `button`.
...
- **A field's fill is one step darker than the surface it sits on.** boardui's input is the tertiary well, and in the dark theme `background-tertiary-default` and `background-primary-default` are the same neutral — a field on a card, a modal or `Sidebar.Main` is invisible until hovered. boardui's own settings page gives a field on the lighter surface the secondary fill, and `Input` / `TextArea` / `SearchField` / `InputGroup` take that as `surface="primary"`, the default; a field inside the sidebar or another secondary panel says `surface="secondary"` to get the tertiary well back.
...
- **The conventions above are gated, not reviewed.** A 2026-09 audit against the
  design-taste profile of the time found 218 violations across 114 files, and the
  ...
  on a `*.Content` slot, `onClick`/`disabled` on Button, every HeroUI token
  name, Tailwind type sizes and bare font weights, a Spinner sized by className,
  a template-string className, `t(…).replace`, glyph icons, px max-widths, native
  `label`/`kbd` — and `scripts/eslint-rules/` is a local plugin for what needs
  more than a selector: an icon-only control needs a `<Tooltip>` ancestor, every
  intrinsic element in app code carries `data-slot`, and a base component may not
  accept a prop and drop it as `_x` (`no-silent-prop-drop` — the shape behind
  every dead primitive of the first boardui pass). Each message names the
  replacement. The base layer is under all of it except `data-slot`.
...
dependencies; nothing is fetched at build time and no key is needed for the
free registry. Pro items need `npx boardui login <key>` once per machine. An
installed file is adapted here (see "UI Conventions") and must not be
re-added with `--overwrite`.
