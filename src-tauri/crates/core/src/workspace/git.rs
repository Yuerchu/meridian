...
/// index". A repository with no commits yet has no `HEAD`; there everything is
/// effectively new, and the per-file untracked path below still works.
pub async fn diff(root: &Path, rel_path: Option<&str>) -> Result<GitDiffResult, String> {
    if let Some(rel) = rel_path
        && !is_tracked(root, rel).await?
    {
    ...
    if out.status != 0 {
        // No commits yet: fall back to diffing against the empty index, which
        // is the only "before" a fresh repository has.
        let mut fallback = vec!["diff", "--no-color"];
        if let Some(rel) = rel_path {
            fallback.push("--");
            fallback.push(rel);
        }
...
async fn run_in(root: &Path, args: &[&str]) -> Result<GitOutput, String> {
    ...
    let run = async {
        ...
        let mut capped_out = out_pipe.take(MAX_OUTPUT_BYTES + 1);
        let (out_read, err_read, waited) = tokio::join!(
            capped_out.read_to_end(&mut stdout),
            err_pipe.read_to_end(&mut stderr),
            child.wait(),
        );
        out_read.map_err(|e| format!("reading git output: {e}"))?;
        err_read.map_err(|e| format!("reading git stderr: {e}"))?;
        let status = waited.map_err(|e| format!("waiting for git: {e}"))?;

        let truncated = stdout.len() as u64 > MAX_OUTPUT_BYTES;
        if truncated {
            stdout.truncate(MAX_OUTPUT_BYTES as usize);
        }
        Ok(GitOutput {
