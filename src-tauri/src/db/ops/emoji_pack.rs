...
use crate::db::models::emoji_pack::{EmojiPack, EmojiPackUpdate, NewEmojiPack};
...
pub fn create_pack(conn: &mut SqliteConnection, new: &NewEmojiPack) -> QueryResult<EmojiPack> {
    diesel::insert_into(emoji_packs::table)
        .values(new)
        .execute(conn)?;
...
pub fn update_pack(
    ...
    id: &str,
    changeset: &EmojiPackUpdate,
) -> QueryResult<EmojiPack> {
    diesel::update(emoji_packs::table.find(id))
        .set(changeset)
        .execute(conn)?;
    emoji_packs::table.find(id).first::<EmojiPack>(conn)
}
...
pub fn count_packs(conn: &mut SqliteConnection) -> QueryResult<i64> {
    use diesel::dsl::count_star;
    emoji_packs::table.select(count_star()).first(conn)
}

pub fn list_packs_for_assistant(
    conn: &mut SqliteConnection,
    assistant_id: &str,
) -> QueryResult<Vec<EmojiPack>> {
    emoji_packs::table
        .inner_join(assistant_emoji_packs::table.on(
            assistant_emoji_packs::pack_id.eq(emoji_packs::id),
        ))
