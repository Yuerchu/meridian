...
pub fn replace_models(
    conn: &mut SqliteConnection,
    pid: &str,
    models: &[NewCachedModel],
) -> QueryResult<()> {
    conn.transaction(|conn| {
        diesel::delete(cached_models::table.filter(cached_models::provider_id.eq(pid)))
            .execute(conn)?;
        diesel::insert_into(cached_models::table)
            .values(models)
            .execute(conn)?;
        Ok(())
    })
