...
pub fn sync_index(conn: &mut SqliteConnection, root: &std::path::Path) -> Result<Vec<Skill>, String> {
    ...
    for meta in &found {
        let source = if crate::agent::is_builtin_skill_dir(&meta.dir_name) { "official" } else { "user" };
        ...
        let display_name =
            existing.as_ref().map(|s| s.display_name.clone()).unwrap_or_else(|| meta.llm_name.clone());
        let display_description = existing.as_ref().and_then(|s| s.display_description.clone());

        db::ops::skill::upsert_skill(conn, &NewSkill {
...
pub fn get_skill_body(app: tauri::AppHandle, dir_name: String) -> Result<String, String> {
    let root = skills_root(&app)?;
    skills::read_skill_body(&root, &dir_name)
...
pub fn create_skill(
    ...
    if let Some(name) = display_name.filter(|n| !n.trim().is_empty()) {
        db::ops::skill::update_skill(&mut conn, &dir_name, &SkillUpdate {
...
pub fn update_skill(
    ...
    if updates.llm_description.is_some() || updates.body.is_some() {
        ...
        let description =
            updates.llm_description.clone().unwrap_or_else(|| current.llm_description.clone());
...
pub fn list_skill_bindings(
    ...
    let pool = app.state::<AppDb>();
    let mut conn = get_conn(&pool.0)?;
    db::ops::skill_binding::list_layer(&mut conn, layer, anchor_id.as_deref())
        .map_err(|e| e.to_string())
}
...
pub fn set_skill_binding(
    ...
    if bound {
        ...
        let count = db::ops::skill_binding::count_layer(&mut conn, layer, anchor_id.as_deref())
            .map_err(|e| e.to_string())?;
        ...
        db::ops::skill_binding::bind(&mut conn, layer, anchor_id.as_deref(), &dir_name)
            .map_err(|e| e.to_string())?;
    } else {
        db::ops::skill_binding::unbind(&mut conn, layer, anchor_id.as_deref(), &dir_name)
            .map_err(|e| e.to_string())?;
    }
...
pub fn seed_builtin_bindings(conn: &mut SqliteConnection) {
    for dir_name in crate::agent::BUILTIN_SKILL_DIRS {
        ...
        let already_seeded = db::ops::preference::get_preference(conn, &key)
            .ok()
            .flatten()
            .is_some();
        if already_seeded {
...
mod tests {
    ...
    fn sync_preserves_a_renamed_display_name() {
        ...
        let mut conn = pool.get().unwrap();
        sync_index(&mut conn, dir.path()).unwrap();
        db::ops::skill::update_skill(&mut conn, "s", &SkillUpdate {
    ...
    fn built_in_skills_are_bound_on_first_launch() {
        ...
        let bound =
            db::ops::skill_binding::list_layer(&mut conn, SkillLayer::Global, None).unwrap();
        for dir_name in crate::agent::BUILTIN_SKILL_DIRS {
    ...
    fn a_failed_bind_is_retried_on_the_next_launch() {
        ...
        let bound =
            db::ops::skill_binding::list_layer(&mut conn, SkillLayer::Global, None).unwrap();
        assert!(bound.iter().any(|b| b == crate::agent::diagnostics::DIAGNOSTICS_DIR), "{bound:?}");
    }
}
