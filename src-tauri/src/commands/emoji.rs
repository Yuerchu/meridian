...
pub async fn suggest_sticker_semantics(app: tauri::AppHandle, id: String) -> Result<Emoji, String> {
    ...
    let secrets = app.state::<crate::state::AppSecrets>().0.clone();
    let data_dir = app.path().app_data_dir().map_err(|e| e.to_string())?;
    ...
    let resolved = crate::agent::resolve_provider_config(&secrets, &pool, assistant.as_ref())?;
    ...
    let response = provider
        .chat(
            vec![crate::provider::ChatMessage::user(&content)],
            crate::provider::ChatParams {
