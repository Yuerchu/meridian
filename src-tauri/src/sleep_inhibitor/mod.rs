...
#[derive(Clone)]
pub(crate) struct AppSleepInhibitor(Arc<Mutex<SleepState>>);
...
impl AppSleepInhibitor {
    ...
    pub(crate) fn begin_turn(&self) -> TurnSleepGuard {
...
pub(crate) struct TurnSleepGuard(Arc<Mutex<SleepState>>);
