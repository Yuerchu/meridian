...
pub(crate) struct EventBus(Arc<Inner>);
...
impl EventBus {
    ...
    pub(crate) fn register(&self, sink: Arc<dyn EventSink>, critical: bool) -> SinkId {
    ...
    pub(crate) fn emit(&self, channel: &str, payload: serde_json::Value) -> Result<(), String> {
...
pub(crate) struct BusEmit(pub(crate) EventBus);
