...
const RAW_TABLES: RawTable[] = [
  ...
  // ── 供应商与模型 ──────────────────────────────────────────────
  {
    ...
    cols: [
      ...
      ['credential_kind', 'TEXT', ['NN'], "'api_key'", '凭证从哪来；不参与适配器选择（迁移 40）'],
      [
        'transport_profile',
        'TEXT',
        ['NN'],
        "'standard'",
        '怎么发请求；与 provider_type + api_format 一起选适配器（迁移 40）',
      ],
    ],
    rels: [
  ...
  // ── 审计与计费 ────────────────────────────────────────────────
  {
    ...
    cols: [
      ...
      [
        ...
        ['NN'],
        "'metered'",
        'metered / subscription / external——这次请求<b>欠不欠单价</b>（迁移 40）。同样是写入时快照',
      ],
    ],
    rels: [
