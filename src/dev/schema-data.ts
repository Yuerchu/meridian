...
const RAW_TABLES: RawTable[] = [
  ...
  },
  {
    ...
    note: '余额告警和用量暴增告警往哪发。<b>只有 <code>generic</code> 是我们自己的契约</b>，另外四种跟着钉钉 / 飞书 / 企业微信 / Slack 各自的上游走——签名算法四家互不相同，且前三家用 HTTP 200 加 body 里的错误码表示失败，所以投递成败不能只看状态码。轮询在 <code>notify::balance</code> 与 <code>notify::usage</code>；出口除了这张表，还有注册进 <code>services.alert_sinks</code> 的 sink（今天只有 OneBot 一个）。',
    cols: [
      ['id', 'TEXT', ['PK', 'NN'], '—', 'UUID'],
      ['name', 'TEXT', ['NN'], '—', '给人看的名字'],
      ['url', 'TEXT', ['NN'], '—', '只允许 http/https；钉钉与企业微信把凭据放在 query 里'],
      ['format', 'TEXT', ['NN'], '—', "'generic' | 'dingtalk' | 'feishu' | 'wecom' | 'slack'"],
    ...
    rels: [
      ...
      '签名密钥<b>不在这张表里</b>：每一行都会作为 <code>InfoResponse</code> 跨 IPC，密钥列等于每次 list 都泄一次。它在钥匙串里，键名 <code>NOTIFY_WEBHOOK_SECRET_&lt;id&gt;</code>，与 provider API key 同一套做法；响应只回 <code>has_secret</code>。',
    ],
    rules: [
      '相关的七个 Tauri 命令全部 <code>local</code>。URL 决定「写着服务商名字和金额的告警」发到哪去，能远程改它的人可以把告警重定向走——这是 <code>acp.command</code> 那一类，不是自锁。',
      '最多 32 个端点：一次告警不该变成上百个出站请求。',
