 ...
 * Providers, as pages.
 *
 * The list is the root, a provider is a page over it, and its models will be a
 * page over that. What used to be here — a selection feeding a two-column
 ...
 */
type ProviderLevel = { kind: 'provider'; providerId: string }
...
export function ProviderSettings() {
  ...
  return (
    <SettingsStack stack={stack}>
      {(level) =>
        level === null ? (
          <ProviderListPage onOpen={(providerId) => stack.push({ kind: 'provider', providerId })} />
        ) : (
          <ProviderPage
            key={level.providerId}
            providerId={level.providerId}
            // Unguarded: the draft it is asking about belongs to a row that no
            // longer exists, so the question would be about nothing.
