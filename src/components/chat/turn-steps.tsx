...
import type { TurnStep } from '@/lib/turns'
    ...
    <div data-slot="turn-steps" className={cn('flex flex-col gap-3', className)}>
      {steps.map((step, i) => {
        ...
        if (step.kind === 'text') {
          ...
          return (
            <div key={key} data-slot="turn-step-text" className="text-xs text-muted-foreground">
              <MarkdownContent content={step.text} oneBot={isOneBot} emojiMap={emojiMap} />
            </div>
          )
        }
        return <MemoToolCallBlock key={key} data={step.data} className="my-0" />
      })}
    </div>
  )
}
