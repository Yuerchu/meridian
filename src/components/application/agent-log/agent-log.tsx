 ...
 */
export function RowConnector({ first, last, reduce }: { first: boolean; last: boolean; reduce: boolean }) {
  return (
    <span aria-hidden className="pointer-events-none absolute inset-y-0 left-0 w-3 text-foreground-icon-quaternary">
      <svg
        width={BRANCH_WIDTH}
        height={BRANCH_Y + 1}
 ...
 */
export function WorkingRow({ label, reduce, className }: { label: string; reduce: boolean; className?: string }) {
