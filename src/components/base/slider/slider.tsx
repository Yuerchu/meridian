...
type SliderBaseProps<T extends SliderValue> = Omit<AriaSliderProps<T>, 'children' | 'className' | 'orientation'> & {
  ...
  return (
    <AriaSlider
      ...
      className={({ isDisabled }) =>
        cx('group flex w-full min-w-0 flex-col gap-2', isDisabled && 'cursor-not-allowed opacity-50', className)
      }
    >
      {({ state }) => {
        ...
        return (
          <>
            {label != null && <AriaLabel className="text-body-medium text-text-primary">{label}</AriaLabel>}

            <AriaSliderTrack className={cx('relative h-8 w-full touch-none', showTooltip && 'mt-8')}>
              {({ isHovered }) => (
                <>
                  <span
                    aria-hidden
                  ...
                  {state.values.map((value, index) => {
                    const valueLabel = formatValue?.(value, index) ?? state.getThumbValueLabel(index)

                    return (
                      <AriaSliderThumb
                        key={index}
                        index={index}
                        aria-label={thumbLabels[index] ?? (isRange ? `Value ${index + 1}` : 'Value')}
                        className={({ isDragging, isHovered: isThumbHovered, isFocusVisible, isDisabled }) =>
                          cx(
                            ...
                            isDragging &&
                              'z-20 scale-110 border-accent-500 shadow-[0_3px_8px_color-mix(in_srgb,var(--color-accent-600)_22%,transparent)]',
...
export function RangeSlider({ thumbLabels = ['Minimum', 'Maximum'], ...props }: RangeSliderProps) {
