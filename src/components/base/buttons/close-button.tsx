...
export function CloseButton({ size = 'xs', className, type = 'button', ref, ...props }: CloseButtonProps) {
  const glyph = GLYPH_SIZE[size]
  const strokeWidth = STROKE_WIDTH[size]
  const inset = GLYPH_INSET[size]

  return (
    <button ref={ref} type={type} className={cx(styles.base, styles.container[size], className)} {...props}>
